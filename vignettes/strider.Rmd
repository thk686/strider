...
The **strided_pointer** and related functions are designed to be used in C++ code. I first create a header to avoid some repetitive code declarations. Unfortunately, Rcpp attributes cannot be used in a header and are included in the code examples below. The dependency on BH is only to fix a problem with Travis CI. For sake of brevity, the header contents are omitted here. They can be found in the original markdown file. All compiler flags are left at default values as determined by R.
...
As a prelude to using the **strided_pointer** class, I start out with an example of vector convolution because it is one of the original examples in the [Rcpp](https://github.com/RcppCore/Rcpp) documentation. The code below appears to be Rcpp's 14th revision of this algorithm fully optimized for speed.
...
// [[Rcpp::export]]
SEXP convolve14cpp(SEXP a, SEXP b){
...
The Fast vector type is a very lightweight wrapper around a data buffer designed to minimize the overhead of integer indexing. The next code block shows my translation using the standard library transform algorithm. This version eschews integer indexes for incrementing iterators along the input and output vectors.
...
// [[Rcpp::export]]
SEXP stl_algo_convolve(SEXP a, SEXP b){
    NumericVector xa(a), xb(b);
    int n_xa = xa.size() ;
    int n_xb = xb.size() ;
    int nab = n_xa + n_xb - 1;
    ...
    transform(begin(xa), end(xa),
              begin(xab), begin(xab),
              [&](const double& t, double& u){
                transform(begin(xb), end(xb), &u, &u,
...
a = sample(1:1e2)
b = sample(1:1e2)
if (!identical(convolve14cpp(a, b), stl_algo_convolve(a, b)))
  stop("stl_algo_convolve is incorrect")
```

While in this particular case, the code is more complicated and a bit more difficult to reason about, replacing explicit loops with standard library algorithms is considered a best-practice that is less susceptible to indexing errors and buffer overruns.
...
a = sample(1:1e3)
b = sample(1:1e3)
microbenchmark(convolve14cpp(a, b), stl_algo_convolve(a, b)) %>%
  summary(unit = "ms") %>% select(expr, median) %>%
  mutate(ratio = median / min(median)) %>%
  arrange(median) %>% knitr::kable(digits = 2)
```

The transform version is perhaps 10% faster than the optimized Rcpp example. (These results will likely change on different computers and with different compilers.) Not a big gain, but nonetheless remarkable for a first try right out of the gate. Generally an iterator-based approach will meet or exceed the efficiency of for-loops with integer offsets. In the following, I show out the **strider** package can assist authors in avoiding explicit for-loops when scanning multidimensional buffers.
...
This code is sub-optimal for a number of reasons. For one, each vector and matrix access requires computing pointer offsets into the underlying buffer. The results below show that this implementation is much slower than R's built-in ```colSums``` function.
...
The results are not bad, matching the timing of R's built-in ```rowSums```, but is still twice as slow as summing columns owing to the skipping over rows. (Timings appear to depend on the particular machine; the strided row sum is relatively much slower on my desktop than on my laptop.)
...
This version is no more efficient than the ```accumulate``` version. Perhaps there is something intrinsically faster about row sums, or, more likely, the compiler handles ```for_each``` very efficiently. Figure 1. plots the observed run-times for comparison.
