...
As a prelude to using the **strided_pointer** class, I start out with an example of vector convolution because it is one of the original examples in the [Rcpp](https://github.com/RcppCore/Rcpp) documentation, which I have copied here.
...
While in this particular case the code is a bit more complicated, replacing explicit loops with standard library algorithms is considered a best-practice that is less susceptible to indexing errors and buffer overruns.
...
a = sample(1:1e3)
b = sample(1:1e3)
microbenchmark(convolve_cpp(a, b), stl_algo_convolve(a, b)) %>%
  summary(unit = "ms") %>% select(expr, median) %>%
  mutate(ratio = median / min(median)) %>%
  arrange(median) %>% knitr::kable(digits = 2)
```

The two algorithms are roughly the same in efficiency. (These results will likely change on different computers and with different compilers.) Generally an iterator-based approach will meet or exceed the efficiency of for-loops with integer offsets while avoiding potential errors in indexing code. In the following, I show out the **strider** package can assist authors in avoiding explicit for-loops when scanning multidimensional buffers.
...
This version is in a dead-heat with R's internal C function ```colSums```.
