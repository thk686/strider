...
```{r include=FALSE}
has_packages = all(c("dplyr", "ggplot2", "microbenchmark") %in% rownames(installed.packages()))
not_has_packages = !has_packages
```

```{r echo=FALSE, eval=not_has_packages}
cat("Error: You must install dplyr, ggplot2 and microbenchmark to run the code in this vignette.")
```
...
```{r echo=FALSE, eval=not_has_packages}
cat("Error: Results omitted. Please install dplyr, ggplot2 and microbenchmark.")
```
...
```{r, include=FALSE, eval=has_packages}
a = 1:100
b = 1:100
...
```{r echo=FALSE, eval=has_packages}
a = rnorm(5e3)
b = rnorm(5e3)
...
Note that [Boost Range](https://www.boost.org/doc/libs/release/libs/range/) and the [range v3 proposal](https://github.com/ericniebler/range-v3) have strided iterators. Also, the [xtensor](https://github.com/QuantStack/xtensor) library appears to be the heir-apparent for working with multidimensional data in C++ and perhaps [R](https://cran.r-project.org/package=xtensor) as well. I have not tried to implement a 2D covolution with these, however it would be an interesting experiment.
