...
There is no particular performance advantage to avoiding indices, so the choice is largely up to taste. Modern compilers will produce similar if not the exact same instructions regardless of the type of loop. Benchmarking comparisons can however point out problems with a particular implementation. It is useful to recall that benchmark differences within $\pm 10$% are likely meaningless and results will vary in different environments. With two vectors of length $10^3$, I get the following (in milliseconds) median run times for the index-based and stl-based implementations.
...
tested. Upon specialization, **strided_pointer** conforms to the **RandomAccessIterator** concept. A second class, **iterator_range** exists solely to construct begin and end iterators over a strided range.
...
The main functions above are ```strided_begin``` and ```strided_end```, which create starting and ending iterators over the columns of data. This is a fairly efficient implementation as shown below.

```{r}
x = matrix(rnorm(1e6), 1e3)
microbenchmark(colSums(x), indx_col_sum(x), strided_col_sum(x)) %>%
  summary(unit = "ms") %>% select(expr, median) %>%
  mutate(ratio = median / min(median)) %>%
  arrange(median) %>% knitr::kable(digits = 2)
```
